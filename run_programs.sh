...
# This script runs clingo and DLV (using Generate and Test programs without cardinality constraints) and writes some statistics in a file
...
function run_dlv {
    reached_timelimit=false
    { time timeout $1s dlv -stats -n=1 $2 $3 > ${tmp_path}/out.txt 2>& 1 ; } 2> ${tmp_path}/time.txt
    success=$?
    # We check if the command was timeout, or not
    if [ ${success} -eq 124 ]
    then
    ...
    do
        ...
        then
            rules=$(echo ${line} | cut -d' ' -f3)
        elif [[ ${line} =~ "Constraints" ]] && [[ ! ${line} =~ "Weak" ]]
        then
            constraints=$(echo ${line} | cut -d' ' -f3)
        elif [[ ${line} =~ "{" ]]
        then
...
min_n=$3
max_n=$4
timeout=${5:-100} # optional argument
echo ${program} ${database_type} ${min_n} ${max_n} ${timeout}

if [ -z "${program}" ]
then
    echo "The first argument must be the program. It must be \"clingo\" or \"dlv\""
    exit 1
fi
if [ "${program}" != 'clingo' ] && [ "${program}" != 'dlv' ]
then
    echo "${program} is not a valid program. It must be \"clingo\" or \"dlv\""
    exit 1
fi
if [ -z "${database_type}" ]
then
...
tmp_path="tmp-${database_type}-${program}"
if [ ! -d ${tmp_path} ]
then
    mkdir ${tmp_path}
fi
...
output_file="generated/dataset_varying_${database_type}_${program}_${min_n}_${max_n}_${n_queries}_${time_limit}.txt"
...
do
    ...
    python3 conquesto/generate_files.py ${database_type} ${program} ${query}
    ...
    do
        ...
        db_size=$(python3 conquesto/generate_files.py ${database_type} ${program} ${query} ${n})
        ...
        do
            ...
            then
                run_dlv ${time_limit} ${program_file} ${db_path}
            fi
